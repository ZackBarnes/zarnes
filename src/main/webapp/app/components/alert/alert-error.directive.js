(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('zarnesApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-zarnesApp-error');
                var entityKey = httpResponse.headers('X-zarnesApp-params');
                if (errorHeader) {
